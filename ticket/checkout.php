...
if (isset($_POST['sub']) && $_POST['sub'] === 'Checkout') {
    $ticket_billing_address = $_POST['ticket_billing_address'];
    $ticket_campus = $_POST['ticket_campus'];
    $ticket_form_id = $_POST['ticket_form_id'];
    ...
    $sql = "INSERT INTO tickets (address, campus) VALUES ('$ticket_billing_address', '$ticket_campus')";
    if ($con->query($sql) === TRUE) {
        $ticket_id = $con->insert_id;
        $sql = "UPDATE ticket_forms SET ticket_id = $ticket_id WHERE id = $ticket_form_id";

        if ($con->query($sql) === TRUE) {
            header("Location: /assignment/ticket/ticket_details.php?ticket_id=$ticket_id");
        }
    }
}
...
$sql = "SELECT * FROM ticket_forms WHERE id = " . $_GET['ticket_form_id'];
$result = $con->query($sql);
$ticket_form = $result->fetch_assoc();

$sql = "SELECT * FROM movie_showtimes WHERE id = " . $_GET['id'];
$result = $con->query($sql);
$movie_showtime = $result->fetch_assoc();

$sql = "SELECT * FROM movies WHERE id = " . $movie_showtime['movie_id'];
$result = $con->query($sql);
...
<head>
    ...
    <title>Checkout - Silver Screen Club</title>
...
<body>
    ...
    <main class="container mx-auto mt-20 px-4 scroll-smooth">
        <form class="w-full flex flex-col-reverse md:flex-row gap-2" action="" method="post">
            <input type="hidden" name="ticket_form_id" value="<?php echo $_GET['ticket_form_id'] ?>">

            <div class="mt-3 md:w-3/5 md:pr-4">
                ...
                <div class="mt-8 flex flex-col md:flex-row md:items-center">
                    <h5 class="mr-4 text-lg font-bold tracking-tight text-gray-900 dark:text-white">
                        Payment Credentials
                    </h5>
                    <div class="flex items-center">
                        <svg aria-hidden="true" class="w-10 h-3 mr-3 -ml-1" viewBox="0 0 660 203" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <svg aria-hidden="true" class="h-4 mr-3 -ml-1 w-7" viewBox="0 0 601 360" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="mt-3">
                    <label for="ticket_card_number" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Card Number</label>
                    <input type="text" id="ticket_card_number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Card Number" required>
                </div>
                <div class="w-full flex flex-col md:flex-row gap-2">
                    <div class="w-full mt-2">
                        <label for="ticket_card_exp" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Card Expiry Date</label>
                        <input type="text" id="ticket_card_exp" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Card Expiry Date" required>
                    </div>
                    <div class="w-full mt-2">
                        <label for="ticket_card_cvv" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Card CVV</label>
                        <input type="text" id="ticket_card_cvv" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Card CVV" required>
                    </div>
                </div>
                ...
                <div class="mt-4 flex justify-between">
                    <div></div>
                    <div>
                        <input type="submit" name="sub" value="Checkout" class="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mb-2" />
                    </div>
                </div>
            </div>
            <div class="mt-3 md:w-2/5">
                <div class="p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Checkout Details</h5>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr class="border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">
                                        RM <?php echo number_format($movie_showtime['price'], 2, '.', ' '); ?>
                                    </th>
                                </tr>
                                <tr class="border-b border-gray-200 dark:border-gray-700">
                                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">
                                    ...
                                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">
                                        <?php echo $movie_showtime['discount_rate']; ?>%
                                    </th>
                                </tr>
                                <tr class="border-y-4 border-gray-200 dark:border-gray-700">
                                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">
                                        Total Price
                                    </th>
                                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">
                                        RM <?php echo number_format($movie_showtime['price'] - ($movie_showtime['price'] * $movie_showtime['discount_rate'] / 100), 2, '.', ' '); ?>
                                    </th>
                                </tr>
                            </tbody>
                        </table>
